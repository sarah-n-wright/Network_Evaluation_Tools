...
This is an advanced developer section that describes creation of a new release for neteval and deploying
...
Be sure to update the version in ``neteval/__init__.py`` Omit the ``v`` when setting the version in the file.

Commit the above changes and push to **main**:

.. code-block::

    git commit -m 'message...' neteval/__init__.py
...
From neteval source tree generate the wheel and tar.gz files
...
.. code-block::
    ...
    $ tree
    dist
    |-- neteval-0.1.0-py2.py3-none-any.whl
    `-- neteval-0.1.0.tar.gz
...
#. From https://github.com/sarah-n-wright/neteval click on releases link.
...
Once the above is done verify deploy was successful by browsing to https://testpypi.python.org/pypi/neteval and verifying new release was deployed.

Also test the package by installing it locally by running this:

.. code-block::

    pip install -i https://testpypi.python.org/pypi neteval
...
.. _Travis: https://travis-ci.org/sarah-n-wright/neteval
.. _main: https://github.com/sarah-n-wright/neteval/tree/main
